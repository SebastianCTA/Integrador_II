...
export class EditNewProductComponent implements OnInit {
  ...
  saveVariedad(){
    if (!this.valor_multiple ||
      !this.stock_multiple) {
        this.toaster.open(NoticyAlertComponent,{text: `danger- 'Upps! Necesita digitar los datos.'`});
        return;
    }
    let data = {
    ...
    this._productService.createVariedad(data).subscribe((resp:any) => {
      ...
      if (index !=-1) {
        ...
        this.toaster.open(NoticyAlertComponent,{text: `primary- 'La variedad se ha editado correctamente.'`});
      }else{
        this.variedades.unshift(resp.variedad)
        this.toaster.open(NoticyAlertComponent,{text: `primary- 'Se registro correctamente la variedad.'`});
      }
    })
  }
  ...
  deleteVariedad(variedad){
    ...
    modalRef.componentInstance.VariedadD.subscribe((resp: any)=>{
      ...
      if (index !=-1) {
        ...
        this.toaster.open(NoticyAlertComponent,{text: `primary- 'La variedad se ha eliminado correctamente.'`});
      }
    })
  }
  storeImagen(){
    if (!this.imagen_file_galeria) {
      this.toaster.open(NoticyAlertComponent,{text: `danger- 'Necesita seleccionar una imagen.'`});
      return;
    }
    let formData = new FormData();
    formData.append("_id",this.product_id);
  ...
  removeImagen(imagen){
    ...
    modalRef.componentInstance.ImagenD.subscribe((resp: any)=>{
      ...
      if (index !=-1) {
        ...
        this.toaster.open(NoticyAlertComponent,{text: `primary- 'La imagen se ha eliminado correctamente.'`});
      }
    })
  }
